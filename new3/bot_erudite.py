...
str_skills = 'Я бот-эрудит. Авторы: Горшков Тимофей, Мендыгалиев Данияр, Барсуков Максим (2026 г.). Вот, что я умею:' \
			 ...
			 '\n⭐ Напиши "погода" и укажи место: например, "Москва". Можно сразу с датой: "Москва завтра" или "Минск 24.12"' \
			 '\n⭐ Напиши "время", "дата", "день недели", "сезон", "месяц". Можно спросить про конкретную дату: "какой день недели 24.12.2027"'\
			 '\n⭐ Напиши "сколько дней в феврале 2028" (или в любом другом месяце), чтобы узнать точное количество дней' \
...
MONTH_WORDS = {
	'январ': 1, 'феврал': 2, 'март': 3, 'апрел': 4, 'май': 5, 'мае': 5,
	'июн': 6, 'июл': 7, 'август': 8, 'сентябр': 9, 'октябр': 10, 'ноябр': 11, 'декабр': 12
}


def weekday(target_date=None): # Узнать день недели (возвращает строку типа "среда")
	target_date = target_date or datetime.date.today()
	return WEEKDAYS[target_date.weekday()]


def month(target_date=None): # Узнать месяц (возвращает строку типа "Августа")
	target_date = target_date or datetime.date.today()
	return MONTHS_GENITIVE[target_date.month]


def month2(target_date=None): # Узнать месяц 2.0 (возвращает строку типа "Август")
	target_date = target_date or datetime.date.today()
	return MONTHS_NOMINATIVE[target_date.month]


def month3(target_date=None): # Узнать месяц 3.0 (возвращает строку типа "Августе")
	target_date = target_date or datetime.date.today()
	return MONTHS_PREPOSITIONAL[target_date.month]


def season(target_date=None): # Узнать сезон (возвращает строку типа "лето")
	target_date = target_date or datetime.date.today()
	m = target_date.month
...
def parse_date_from_text(text):
	if not text:
		return None
	vksms = text.lower().strip()
	today = datetime.date.today()
	if 'сегодня' in vksms:
		return today
	if 'завтра' in vksms:
		return today + datetime.timedelta(days=1)
	if 'послезавтра' in vksms:
		return today + datetime.timedelta(days=2)
	if 'вчера' in vksms:
		return today - datetime.timedelta(days=1)

	numeric_date = re.search(r'(\d{1,2})[./-](\d{1,2})(?:[./-](\d{2,4}))?', vksms)
	if numeric_date:
		day = int(numeric_date.group(1))
		month_num = int(numeric_date.group(2))
		year = int(numeric_date.group(3)) if numeric_date.group(3) else today.year
		if year < 100:
			year += 2000
		try:
	...
	month_num = None
	for month_key, month_value in MONTH_WORDS.items():
		if month_key in vksms:
			month_num = month_value
			break
	if month_num:
		day_match = re.search(r'\b([0-2]?\d|3[0-1])\b', vksms)
		year_match = re.search(r'\b(20\d{2}|19\d{2})\b', vksms)
		if day_match:
...
def format_date(target_date):
	return str(target_date.day) + ' ' + month(target_date) + ' ' + str(target_date.year)
...
def build_date_info_message(text):
	target_date = parse_date_from_text(text)
	if not target_date:
		target_date = datetime.date.today()
	return 'Это ' + format_date(target_date) + ', ' + weekday(target_date) + '. Сезон: ' + season(target_date)
			...
			if event.type == VkEventType.MESSAGE_NEW and event.to_me and event.text and event.from_user:
				...
				if idslov[event.user_id] == 'menu':
					...
					if vksms == 'начать':
						...
						tt_h = (now.hour - uuu) % 24
						tt_m = str((now.minute - uum) % 60).zfill(2)
						vk.messages.send(user_id=event.user_id, message='Сейчас ' + str(tt_h) + ':' + tt_m + '\n' + build_date_info_message(vksms) + sub, random_id=get_random_id(), keyboard=keyboard.get_keyboard())
					elif vksms.count('день') > 0 and vksms.count('недел') > 0:
						target_date = parse_date_from_text(vksms) or datetime.date.today()
						vk.messages.send(user_id=event.user_id, message='День недели: ' + weekday(target_date) + '\nДата: ' + format_date(target_date) + sub, random_id=get_random_id(), keyboard=keyboard.get_keyboard())
					elif (vksms.count('какой') > 0 and vksms.count('день') > 0) or vksms.count('дата') > 0:
						vk.messages.send(user_id=event.user_id, message=build_date_info_message(vksms) + sub, random_id=get_random_id(), keyboard=keyboard.get_keyboard())
					elif vksms.count('сезон') > 0:
						target_date = parse_date_from_text(vksms) or datetime.date.today()
						vk.messages.send(user_id=event.user_id, message='В ' + month3(target_date) + ' обычно ' + season(target_date) + '\n(' + format_date(target_date) + ')' + sub, random_id=get_random_id(), keyboard=keyboard.get_keyboard())
					elif vksms.count('месяц') > 0:
						target_date = parse_date_from_text(vksms) or datetime.date.today()
						vk.messages.send(user_id=event.user_id, message='Месяц: ' + month2(target_date) + '\nДата: ' + format_date(target_date) + sub, random_id=get_random_id(), keyboard=keyboard.get_keyboard())
					elif (vksms.count('сколько') > 0 or vksms.count('скок') > 0) and vksms.count('дней') > 0 and vksms.count('в') > 0:
						target_date = parse_date_from_text(vksms)
						if target_date:
							days_count = calendar.monthrange(target_date.year, target_date.month)[1]
							vk.messages.send(user_id=event.user_id, message='В ' + month3(target_date) + ' ' + str(target_date.year) + ' года ' + str(days_count) + ' дней' + sub, random_id=get_random_id(), keyboard=keyboard.get_keyboard())
						else:
							month_num = extract_month_from_text(vksms)
							if month_num:
								year_match = re.search(r'\b(20\d{2}|19\d{2})\b', vksms)
								year = int(year_match.group(1)) if year_match else datetime.date.today().year
								days_count = calendar.monthrange(year, month_num)[1]
								vk.messages.send(user_id=event.user_id, message='В ' + MONTHS_PREPOSITIONAL[month_num] + ' ' + str(year) + ' года ' + str(days_count) + ' дней' + sub, random_id=get_random_id(), keyboard=keyboard.get_keyboard())
							else:
								vk.messages.send(user_id=event.user_id, message='Напиши месяц или дату (например: "сколько дней в феврале 2028" или "сколько дней в 02/2027")' + sub, random_id=get_random_id(), keyboard=keyboard.get_keyboard())
					...
					# Вызов Погоды
						...
						keyboard = VkKeyboard(one_time=True)
						keyboard.add_button('Отмена', color=VkKeyboardColor.NEGATIVE)
						vk.messages.send(user_id=event.user_id, message='Напиши город/страну. Можно сразу с датой: "Москва завтра", "Минск 24.12"', random_id=get_random_id(),
				...
				# Погода
					...
					if vksms == 'выйти' or vksms == 'отмена':
						...
						try:
							query_date = parse_date_from_text(vksms)
							place = re.sub(r'(\d{1,2}[./-]\d{1,2}(?:[./-]\d{2,4})?)|сегодня|завтра|послезавтра|вчера', '', vksms).strip(' ,.')
							if not place:
								place = vksms
							owm = pyowm.OWM('2b81034cf1e96c904e721b0da1ad3f9d', config_dict)
							mgr = owm.weather_manager()
							if query_date and query_date > datetime.date.today():
								forecast = mgr.forecast_at_place(place, '3h').forecast
								target_dt = datetime.datetime.combine(query_date, datetime.time(hour=12))
								closest_weather = min(forecast.weathers, key=lambda w: abs((w.reference_time('date') - target_dt).total_seconds()))
								status = closest_weather.detailed_status.capitalize()
								temp = round(closest_weather.temperature('celsius').get('temp'))
								feels = round(closest_weather.temperature('celsius').get('feels_like'))
								Weather = 'Прогноз для ' + place.title() + ' на ' + format_date(query_date) + ':\n' + status + ', ' + str(temp) + '°C (ощущается как ' + str(feels) + '°C)'
							else:
								w1 = mgr.weather_at_place(place)
								w2 = w1.weather
								status = w2.detailed_status.capitalize()
								temp = round(w2.temperature('celsius').get('temp'))
								feels = round(w2.temperature('celsius').get('feels_like'))
								hum = w2.humidity
								wind = round(w2.wind().get('speed', 0), 1)
								Weather = 'Погода в ' + place.title() + ':\n' + status + '\n🌡 Температура: ' + str(temp) + '°C (ощущается как ' + str(feels) + '°C)\n💧 Влажность: ' + str(hum) + '%\n💨 Ветер: ' + str(wind) + ' м/с'
							idslov[event.user_id] = 'menu'
							...
							keyboard = VkKeyboard(one_time=True)
							keyboard.add_button('Отмена', color=VkKeyboardColor.NEGATIVE)
							Weather = 'Не получилось найти погоду. Попробуйте формат: "Москва" или "Москва завтра"'
